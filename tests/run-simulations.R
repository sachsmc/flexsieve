...
truestuff <- true.curves(b$b1, b$b2, b$g1, b$g2)
...
msd <- function(x, ...) {
	me <- median(x)
	2 * median(abs(x - me)) / (qnorm(.75) - qnorm(.25))
}
...
p1 <- ggplot(allscens, aes(x = s, y = drd.s)) +
    geom_smooth(se = FALSE, color = "black") +
    geom_line(data = qnat, aes(x = s.cat.val, y = upper.95.drd), color = "black", linetype = 2) +
    geom_line(data = qnat, aes(x = s.cat.val, y = lower.95.drd), color = "black", linetype = 2) +
    geom_line(data = truescens, aes(x = s, y = drd.s), color = "grey70", linetype = 3, size = 1.5) +
    facet_wrap(~ scenario) + ggtitle("DRD curve") + theme_bw() + ylab(expression(DRD(s[1]))) + xlab(expression(s[1]))
ggsave(file ="drd-curves.png", width = 6.5, height = 5.5)


p2 <- ggplot(allscens, aes(x = s, y = rd.s)) +
    geom_smooth(se = FALSE, color = "black") +
    geom_line(data = qnat, aes(x = s.cat.val, y = upper.95.rd), color = "black", linetype = 2) +
    geom_line(data = qnat, aes(x = s.cat.val, y = lower.95.rd), color = "black", linetype = 2) +
    geom_line(data = truescens, aes(x = s, y = rd.s), color = "grey70", linetype = 3, size = 1.5) +
    facet_wrap(~ scenario) + ggtitle("Risk difference") + theme_bw() + ggtitle("RD curve")  +
    ylab(expression(RD(s[1]))) + xlab(expression(s[1]))
ggsave(file ="rd-curves.png", width = 6.5, height = 5.5)

library(patchwork)
p1 + p2
ggsave(file = "combined-sim-plots.pdf", width = 9.5, height = 4.75)
